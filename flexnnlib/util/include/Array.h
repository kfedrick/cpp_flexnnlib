...
template<class T> inline Vector<T>& Vector<T>::operator+=(const vector<T>& vec)
{
...
public:
   ...
   vector<T>& operator[](unsigned int rowIdx);
   const vector<T>& operator[](unsigned int rowIdx) const;
...
template<class T> inline vector<T>& Array<T>::operator[](unsigned int rowIdx)
{
   return data[rowIdx];
}

template<class T> inline const vector<T>& Array<T>::operator[](
...
void Array<T>::resize(unsigned int rows, unsigned int cols, T fill)
{
   ...
   /*
    * Resize vector of rows. If this increases the number of rows it should
    * fill in the new rows with the default type of vector<T>
    */
   data.resize(rows);
...
public:
   ...
   vector<vector<T> >& operator[](unsigned int XIdx);
   const vector<vector<T> >& operator[](unsigned int XIdx) const;
...
template<class T> inline vector<vector<T> >& Array3D<T>::operator[](
      unsigned int XIdx)
{
   return data[XIdx];
}

template<class T> inline const vector<vector<T> >& Array3D<T>::operator[](
