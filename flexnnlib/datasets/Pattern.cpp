...
Pattern::Pattern(const vector<unsigned int>& vsizes)
{
   this->data.resize(vsizes.size());
   for (unsigned int ndx = 0; ndx < vsizes.size(); ndx++)
      data[ndx] = vector<double>(vsizes[ndx]);

   coalesced_flag = false;
}

Pattern::Pattern(const vector<double>& vec)
{
   this->data.push_back(vec);
   coalesced_flag = false;
}

Pattern::Pattern(const vector< vector<double> >& vec)
{
   this->data = vec;
   coalesced_flag = false;
}
...
const vector<double>& Pattern::operator()(void) const
{
   coalesce();
   return coalesced_data;
}

const vector<double>& Pattern::at(int index) const
{
   return data.at(index);
}

const vector<double>& Pattern::operator[](int index) const
{
   return data[index];
}
...
void Pattern::operator=(const vector< vector<double> >& vec)
{
   this->data = vec;
   coalesced_flag = false;
}

void Pattern::operator=(const vector<double>& vec)
{
   this->data.clear();
   this->data.push_back(vec);
   coalesced_flag = false;
...
void Pattern::push_back(const vector<double>& vec)
{
   this->data.push_back(vec);
}
...
Pattern::operator const vector<double>&() const
{
   coalesce();
   return coalesced_data;
}

void Pattern::toFile(fstream& _fs)
{
   ...
   {
      const vector<double>& pvec = data.at(pndx);
...
void Pattern::fromFile(fstream& _fs)
{
   data.clear();
...
void Pattern::coalesce() const
{
   ...
   for (i=0; i<data.size(); i++)
   {
      const vector<double>& vec = data[i];
