...
#ifndef FLEX_NEURALNET_TDC0TRAINER_H_
#define FLEX_NEURALNET_TDC0TRAINER_H_
...
class TDC0Trainer: public BaseTrainer
{
public:
   TDC0Trainer(BaseNeuralNet& _net);
   virtual ~TDC0Trainer();
...
private:
   ...
   void calc_layer_weight_adj(const BaseLayer& layer, unsigned int timeStep,
         ...
         double V, const vector<double>& S, const vector<double>& prevS);
...
template<class EFunc, class LRPolicy>
TDC0Trainer<EFunc, LRPolicy>::TDC0Trainer(BaseNeuralNet& _net) :
...
template<class EFunc, class LRPolicy>
TDC0Trainer<EFunc, LRPolicy>::~TDC0Trainer()
{
   // TODO Auto-generated destructor stub
}

template<class EFunc, class LRPolicy>
void TDC0Trainer<EFunc, LRPolicy>::alloc_network_learning_rates()
{
...
template<class EFunc, class LRPolicy>
BaseNeuralNet& TDC0Trainer<EFunc, LRPolicy>::get_neuralnet()
{
   return neural_network;
}

template<class EFunc, class LRPolicy>
EFunc& TDC0Trainer<EFunc, LRPolicy>::get_errorfunc()
{
   return error_func;
}

template<class EFunc, class LRPolicy>
void TDC0Trainer<EFunc, LRPolicy>::set_slow_learning_rate_multiplier(
      double _val)
{
...
template<class EFunc, class LRPolicy>
LRPolicy& TDC0Trainer<EFunc, LRPolicy>::get_learning_rates()
{
...
template<class EFunc, class LRPolicy>
void TDC0Trainer<EFunc, LRPolicy>::set_lambda(double l)
{
   lambda = l;
}

template<class EFunc, class LRPolicy>
void TDC0Trainer<EFunc, LRPolicy>::set_gamma(double g)
{
   gamma = g;
}

template<class EFunc, class LRPolicy>
void TDC0Trainer<EFunc, LRPolicy>::set_predict_final_cost()
{
   predict_mode = FINAL_COST;
}

template<class EFunc, class LRPolicy>
void TDC0Trainer<EFunc, LRPolicy>::set_predict_cumulative_cost()
{
   predict_mode = CUMULATIVE_COST;
}

template<class EFunc, class LRPolicy>
void TDC0Trainer<EFunc, LRPolicy>::set_print_gradient(bool _val)
{
   print_gradient = _val;
}

template<class EFunc, class LRPolicy>
void TDC0Trainer<EFunc, LRPolicy>::train(
...
template<class EFunc, class LRPolicy>
double TDC0Trainer<EFunc, LRPolicy>::train_exemplar(
      ...
      else
      {
         ...
         for (unsigned int ndx = 0; ndx < network_layers.size(); ndx++)
         {
            ...
            calc_layer_weight_adj(layer, (unsigned int) 1, layer_deltas.biases,
                  layer_deltas.weights, td_error, opatt().at(0), ipatt(),
                  prev_ipatt());
         }
      }
...
template<class EFunc, class LRPolicy>
void TDC0Trainer<EFunc, LRPolicy>::calc_layer_weight_adj(const BaseLayer& layer,
      unsigned int timeStep, vector<double>& biasDelta,
      Array<double>& weightDelta, double td_error, double V,
      const vector<double>& S, const vector<double>& prevS)
{
   ...
   for (unsigned int in_ndx = 0; in_ndx < layer_input_size; in_ndx++)
   {
      ...
      phi_w += prevS.at(in_ndx) * w.at(in_ndx + 1);
   }
   //cout << "]" << endl;
   ...
   w.at(0) = prev_w.at(0)
         + slow_lr_multiplier * layer_weights_lr.at(0, 0) * (td_error - phi_w);
   for (unsigned int in_ndx = 0; in_ndx < layer_input_size; in_ndx++)
   {
      /*
      ...
      w.at(in_ndx + 1) = prev_w.at(in_ndx + 1)
            + slow_lr_multiplier * layer_weights_lr.at(0, in_ndx)
                  * (td_error - phi_w) * prevS.at(in_ndx);

      theta_p.at(in_ndx) = td_error * prevS.at(in_ndx)
            - gamma * S.at(in_ndx) * phi_w;
 ...
 */
template<class EFunc, class LRPolicy>
void TDC0Trainer<EFunc, LRPolicy>::init_train(
      const DataSet<Exemplar<PatternSequence, PatternSequence> >& _trnset,
      const DataSet<Exemplar<PatternSequence, PatternSequence> >& _vldset,
      const DataSet<Exemplar<PatternSequence, PatternSequence> >& _tstset)
{
 ...
 */
template<class EFunc, class LRPolicy>
void TDC0Trainer<EFunc, LRPolicy>::init_training_epoch()
{
   neural_network.clear_error();
   zero_delta_network_weights();
   save_weights(failback_weights_id);
...
template<class EFunc, class LRPolicy>
TrainingRecord::Entry& TDC0Trainer<EFunc, LRPolicy>::calc_performance_data(
      unsigned int _epoch,
      const DataSet<Exemplar<PatternSequence, PatternSequence> >& _trnset,
      const DataSet<Exemplar<PatternSequence, PatternSequence> >& _vldset,
      const DataSet<Exemplar<PatternSequence, PatternSequence> >& _tstset)
...
template<class EFunc, class LRPolicy>
double TDC0Trainer<EFunc, LRPolicy>::sim(
      const DataSet<Exemplar<PatternSequence, PatternSequence> >& _dataset)
{
   long epoch_no;
   unsigned int trainset_ndx;
...
template<class EFunc, class LRPolicy>
bool TDC0Trainer<EFunc, LRPolicy>::InvertMatrix(const boost::numeric::ublas::matrix<double>& input, boost::numeric::ublas::matrix<double>& inverse)
{
...
template<class EFunc, class LRPolicy>
double TDC0Trainer<EFunc, LRPolicy>::sim2(
      const DataSet<Exemplar<PatternSequence, PatternSequence> >& _dataset)
{
   long epoch_no;
   unsigned int trainset_ndx;
