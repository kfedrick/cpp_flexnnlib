      ...
      public:
            ...
            defaultValueLength_(0)
         {
            ...
            if (properties && properties->IsObject())
            {
               ...
               for (ConstMemberIterator itr = properties->MemberBegin(); itr != properties->MemberEnd(); ++itr)
               {
                  ...
                  if (FindPropertyIndex(itr->name, &index))
                     schemaDocument->CreateSchema(&properties_[index].schema, q.Append(itr->name, allocator_), itr
                        ->value, document);
               }
            }
            ...
            if (const ValueType* v = GetMember(value, GetPatternPropertiesString()))
            {
               ...
               patternProperties_ = static_cast<PatternProperty*>(allocator_
                  ->Malloc(sizeof(PatternProperty) * v->MemberCount()));
               ...
               for (ConstMemberIterator itr = v->MemberBegin(); itr != v->MemberEnd(); ++itr)
               {
                  ...
                  schemaDocument->CreateSchema(&patternProperties_[patternPropertyCount_].schema, q
            ...
            if (dependencies && dependencies->IsObject())
            {
               ...
               for (ConstMemberIterator itr = dependencies->MemberBegin(); itr != dependencies->MemberEnd(); ++itr)
               {
                  ...
                  if (FindPropertyIndex(itr->name, &sourceIndex))
                  {
                     if (itr->value.IsArray())
                     {
                        properties_[sourceIndex].dependencies = static_cast<bool*>(allocator_
                           ->Malloc(sizeof(bool) * propertyCount_));
                     ...
                     else if (itr->value.IsObject())
                     {
                        ...
                        schemaDocument->CreateSchema(&properties_[sourceIndex].dependenciesSchema, q
                           .Append(itr->name, allocator_), itr->value, document);
            ...
            if (const ValueType* v = GetMember(value, GetAdditionalPropertiesString()))
            {
               if (v->IsBool())
                  ...
                  schemaDocument->CreateSchema(&additionalPropertiesSchema_, p
                     .Append(GetAdditionalPropertiesString(), allocator_), *v, document);
            }
            ...
            if (const ValueType* v = GetMember(value, GetAdditionalItemsString()))
            {
               if (v->IsBool())
                  ...
                  schemaDocument->CreateSchema(&additionalItemsSchema_, p
                     .Append(GetAdditionalItemsString(), allocator_), *v, document);
            }
         ...
         bool StartObject(Context& context) const
         {
            ...
            { // pre-allocate schema array
               ...
               context.patternPropertiesSchemas = static_cast<const SchemaType**>(context.factory
                  .MallocState(sizeof(const SchemaType*) * count));
            ...
            if (additionalPropertiesSchema_)
            {
               if (additionalPropertiesSchema_ && context.patternPropertiesSchemaCount > 0)
               {
                  context.patternPropertiesSchemas[context
                     .patternPropertiesSchemaCount++] = additionalPropertiesSchema_;
                  context.valueSchema = typeless_;
   ...
   public:
      ...
      */
         ...
         schemaRef_(allocator, kInitialSchemaRefSize)
      {
         ...
         new(typeless_) SchemaType(this, PointerType(), ValueType(kObjectType).Move(), ValueType(kObjectType)
            .Move(), allocator_);
         ...
         while (!schemaRef_.Empty())
         {
            ...
            if (const SchemaType* s = GetSchema(refEntry->target))
            {
               ...
               {
                  new(schemaMap_.template Push<SchemaEntry>()) SchemaEntry(refEntry
                                                                              ->source, const_cast<SchemaType*>(s), false, allocator_);
               }
            }
   ...
   private:
      ...
      CreateSchema(const SchemaType** schema, const PointerType& pointer, const ValueType& v, const ValueType& document)
      {
         ...
         if (v.IsObject())
         {
            ...
            {
               SchemaType* s = new(allocator_
                                      ->Malloc(sizeof(SchemaType))) SchemaType(this, pointer, v, document, allocator_);
            ...
            if (len > 0)
            {
                        ...
                        if (pointer.IsValid())
                        {
                           if (const SchemaType* sc = remoteDocument->GetSchema(pointer))
                           {
                              if (schema)
                                 *schema = sc;
                              new(schemaMap_
                                     .template Push<SchemaEntry>()) SchemaEntry(source, const_cast<SchemaType*>(sc), false, allocator_);
                              return true;
                           }
                        }
                     }
               ...
               { // Local reference, defer resolution
                  ...
                  if (pointer.IsValid())
                  {
                     ...
                     new(schemaRef_
                            .template Push<SchemaRefEntry>()) SchemaRefEntry(source, pointer, schema, allocator_);
                     return true;
                  }
               }
            }
   ...
   public:
      ...
      */
      ...
      void DoesNotMatch(const Ch* str, SizeType length)
      {
         ...
         currentError_
            .AddMember(GetActualString(), ValueType(str, length, GetStateAllocator()).Move(), GetStateAllocator());
...
#undef RAPIDJSON_SCHEMA_HANDLE_VALUE_
      ...
      {
         return new(GetStateAllocator()
                       .Malloc(sizeof(GenericSchemaValidator))) GenericSchemaValidator(*schemaDocument_, root, documentStack_
                                                                                          .template Bottom<char>(), documentStack_.GetSize(),
#if RAPIDJSON_SCHEMA_VERBOSE
            depth_ + 1,
#endif
                                                                                       &GetStateAllocator());
   ...
   private:
         ...
         else
         {
            if (CurrentContext().inArray)
               internal::TokenHelper<internal::Stack<StateAllocator>,
                                     Ch>::AppendIndexToken(documentStack_, CurrentContext().arrayElementIndex);
      ...
      bool EndValue()
      {
         ...
         *documentStack_.template Push<Ch>() = '\0';
         ...
         uint64_t h = CurrentContext().arrayUniqueness ? static_cast<HasherType*>(CurrentContext().hasher)
            ->GetHashCode() : 0;
         ...
         if (!schemaStack_.Empty())
         {
            ...
            if (context.valueUniqueness)
            {
               ...
               if (!a)
                  CurrentContext().arrayElementHashCodes = a = new(GetStateAllocator()
                                                                      .Malloc(sizeof(HashCodeArray))) HashCodeArray(kArrayType);
                          ...
                          const typename SchemaType::ValueType& (* exclusive)() = 0)
      {
         ...
         currentError_
            .AddMember(GetExpectedString(), ValueType(expected, GetStateAllocator()).Move(), GetStateAllocator());
         if (exclusive)
