...
TEST_P(TestRadBasActivation, Activate)
{
   printf("TEST Case %s\n", GetParam());
   ...
   for (auto& item : test_case.samples)
   {
      ...
      // Check dy_dnet
      EXPECT_PRED3(array_double_near, item.target.dAdN, test_case.layer_ptr->get_dy_dnet(), 0.000000001) << "ruh roh";
...
TEST_P(TestRadBasActivation, ActivateTwice)
{
   printf("TEST Case %s\n", GetParam());
   ...
   std::cout << "no samples = " << test_case.samples.size() << "\n" << std::flush;
