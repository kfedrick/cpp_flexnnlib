...
// at end of room.
template<class State=RawFeature<(10+2)*(10+1)>, class Action=SteeringActionFeature, size_t N=1>
class DerbySim2 : public flexnnet::Environment<State, Action, N>
{
public:
   const unsigned int ROOM_WIDTH = 10;
   const unsigned int ROOM_LENGTH = 4;
   const unsigned int DOOR_POS = 10/2;
   const double GAIN{0.25};

public:
   DerbySim2();
   DerbySim2(const DerbySim2<State,Action,N>& _sim);
...
template<class S, class A, size_t N>
inline
DerbySim2<S,A,N>::DerbySim2(const DerbySim2<S,A,N>& _sim)
{
   x_pos = _sim.x_pos;
   y_pos = _sim.y_pos;
...
template<class S, class A, size_t N>
inline
size_t DerbySim2<S,A,N>::size(void) const
{
...
template<class S, class A, size_t N>
inline
const flexnnet::Reinforcement<N>& DerbySim2<S,A,N>::get_reinforcement() const
{
   double r;
...
template<class S, class A, size_t N>
inline
bool DerbySim2<S,A,N>::is_terminal(void) const
{
...
template<class S, class A, size_t N>
inline
const S& DerbySim2<S,A,N>::reset(void)
{
   // Initialize position
...
template<class S, class A, size_t N>
inline
void DerbySim2<S,A,N>::set(unsigned int x, unsigned int y)
{
   x_pos = x;
   y_pos = y;
   stale_state = true;
...
template<class S, class A, size_t N>
inline
const S& DerbySim2<S,A,N>::next(const typename A::ActionEnum& _action)
{
   //std::cout << "DerbySim2::next " << x_pos << "\n" << std::flush;
   y_pos++;
   ...
   //std::cout << "DerbySim2::next x_pos " << x_pos << "\n" << std::flush;
...
template<class S, class A, size_t N>
inline
void DerbySim2<S,A,N>::update_state_vector() const
{
...
template<class S, class A, size_t N>
inline
void DerbySim2<S,A,N>::encode_naive_state_vector() const
{
   if (!stale_state)
      return;

   state_vector = -1.0;
   state_vector[y_pos * (ROOM_WIDTH + 2) + x_pos] = 1.0;
...
template<class S, class A, size_t N>
inline
void DerbySim2<S,A,N>::encode_kcoded_state_vector() const
{
   if (!stale_state)
      return;
   ...
   for (int y=0; y <= ROOM_LENGTH; y++)
      ...
      {
         double dist = pow((double) x-x_pos,2) + pow((double) y-y_pos,2);
         state_vector[y * (ROOM_WIDTH + 2) + x] = 2 * exp(-dist) - 1;
      }
   stale_state = false;
}
...
template<class S, class A, size_t N>
inline
const S& DerbySim2<S,A,N>::state() const
{
   update_state_vector();
