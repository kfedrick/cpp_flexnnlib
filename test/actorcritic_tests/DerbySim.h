...
// at end of room.
template<class State=RawFeature<23>, class Action=SteeringActionFeature, size_t N=1>
class DerbySim : public flexnnet::Environment<State, Action, N>
{
public:
...
public:
   DerbySim();
   DerbySim(const DerbySim<State,Action,N>& _sim);
...
template<class S, class A, size_t N>
inline
DerbySim<S,A,N>::DerbySim(const DerbySim<S,A,N>& _sim)
{
   x_pos = _sim.x_pos;
   y_pos = _sim.y_pos;
...
template<class S, class A, size_t N>
inline
size_t DerbySim<S,A,N>::size(void) const
{
...
template<class S, class A, size_t N>
inline
const flexnnet::Reinforcement<N>& DerbySim<S,A,N>::get_reinforcement() const
{
   double r;
...
template<class S, class A, size_t N>
inline
bool DerbySim<S,A,N>::is_terminal(void) const
{
...
template<class S, class A, size_t N>
inline
const S& DerbySim<S,A,N>::reset(void)
{
   // Initialize position
...
template<class S, class A, size_t N>
inline
void DerbySim<S,A,N>::set(unsigned int x, unsigned int y)
{
   x_pos = x;
   y_pos = y;
   stale_state = true;
...
template<class S, class A, size_t N>
inline
const S& DerbySim<S,A,N>::next(const typename A::ActionEnum& _action)
{
...
template<class S, class A, size_t N>
inline
void DerbySim<S,A,N>::update_state_vector() const
{
...
template<class S, class A, size_t N>
inline
void DerbySim<S,A,N>::encode_naive_state_vector() const
{
   if (!stale_state)
      return;
...
template<class S, class A, size_t N>
inline
void DerbySim<S,A,N>::encode_kcoded_state_vector() const
{
   if (!stale_state)
      return;
...
template<class S, class A, size_t N>
inline
const S& DerbySim<S,A,N>::state() const
{
   update_state_vector();
