...
template<typename T> Array2D<double> TestFanoutFFNNActivation<T>::parse_weights (const rapidjson::Value &_obj, size_t _rows, size_t _cols)
{
...
template<typename T> Datum TestFanoutFFNNActivation<T>::parse_datum (const rapidjson::Value &_obj)
{
...
TYPED_TEST_P (TestFanoutFFNNActivation, FanoutFFNNActivation)
{
   ...
   for (auto test_case : test_cases)
   {
      ...
      Datum netout = TestFanoutFFNNActivation<TypeParam>::nnet->activate(test_case.input);
...
REGISTER_TYPED_TEST_CASE_P(TestFanoutFFNNActivation, FanoutFFNNActivation);
INSTANTIATE_TYPED_TEST_CASE_P(My, TestFanoutFFNNActivation, MyTypes);
