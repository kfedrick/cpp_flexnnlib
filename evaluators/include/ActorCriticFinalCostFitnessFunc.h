//
// Created by kfedrick on 3/27/22.
//

#ifndef FLEX_NEURALNET_ACTORCRITICFINALCOSTFITNESSFUNC_H_
#define FLEX_NEURALNET_ACTORCRITICFINALCOSTFITNESSFUNC_H_
...
namespace flexnnet
{
   template<class State,
      ...
      class Env>
   class ActorCriticFinalCostFitnessFunc
   {
   ...
   public:
      ...
      double calc_tde_gradient(
         const RTyp& _R_t1, const RTyp& _Re_t0, ValarrMap& _err, double _E = 1);
   ...
   template<class State,
      class Action, unsigned int RSZ, template<class, class, unsigned int>
      class Env>
   void ActorCriticFinalCostFitnessFunc<State, Action, RSZ, Env>::set_subsample_count(unsigned int _sz)
   {
      if (_sz == 0)
   ...
   template<class State,
      class Action, unsigned int RSZ, template<class, class, unsigned int>
      class Env> double ActorCriticFinalCostFitnessFunc<State, Action, RSZ, Env>::calc_fitness(
   ...
   double ActorCriticFinalCostFitnessFunc<State, Action, RSZ, Env>::evaluate_sample(
      NNTyp& _nnet, EnvTyp& _env)
   {
      double sum_tde;
      ...
      while (!_env.is_terminal())
      {
         ...
         // TODO - for not assume no reinforecment signal until terminal state
         sum_tde = calc_tde_gradient(Re_t1, Re_t0, tde_gradient);
      ...
      sum_tde = calc_tde_gradient(last_R, Re_t0, tde_gradient);
      tde_cache.push_back(sum_tde);
   ...
   template<class State,
      class Action, unsigned int RSZ, template<class, class, unsigned int>
      class Env> double ActorCriticFinalCostFitnessFunc<State, Action, RSZ, Env>::calc_tde_gradient(
      const RTyp& _R_t1, const RTyp& _Re_t0, ValarrMap& _err, double _E)
   {
      ...
      for (const auto& R: R_vamap)
      {
         ...
         _err[id] = R.second - Re_t0_vamap.at(id);
   ...
   template<class State,
      class Action, unsigned int RSZ, template<class, class, unsigned int>
      class Env> std::tuple<double, double> ActorCriticFinalCostFitnessFunc<State,
                                                                      Action,
                                                                      RSZ,
...
#endif // FLEX_NEURALNET_ACTORCRITICFINALCOSTFITNESSFUNC_H_
