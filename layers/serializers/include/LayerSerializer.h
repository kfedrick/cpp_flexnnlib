...
namespace flexnnet
{
   ...
   {
      friend std::map<std::string, std::shared_ptr<NetworkLayer>> BasicNeuralNetSerializer::parseNetworkLayers(const rapidjson::Value& _obj);
   ...
   template<class _LayerType> std::shared_ptr<_LayerType> flexnnet::LayerSerializer<_LayerType>::parse (const std::string &_json)
   {
