...
namespace flexnnet
{
   ...
   public:
      ...
      pointer operator->()
      { return ptr; }
      bool operator==(const self_ref& rhs)
      { return ptr == rhs.ptr; }
      bool operator!=(const self_ref& rhs)
      { return ptr != rhs.ptr; }
      ptrdiff_t operator-(const basic_bidirectional_iterator<T>& _iter)
      { return std::distance(ptr, _iter.ptr); }
   private:
      pointer ptr;
   };
   ...
   public:
      ...
      { return ptr != rhs.ptr; }
      self_ref begin()
      { return self_ref(ptr, 0); }
      self_ref end()
      { return self_ref(ptr, sz); }
      ptrdiff_t operator-(const basic_bidirectional_iterator<T>& _iter)
      { return std::distance(ptr, _iter.ptr); }
   private:
      pointer ptr;
