...
namespace flexnnet
{
   ...
   public:
      ...
      double final_trainingset_performance() const
      {
         if (!training_record.empty())
            return training_record.end()->training_perf();
         else
            return 0;
      }

      double final_validationset_performance() const
      {
         if (!training_record.empty())
            return training_record.end()->validation_perf();
         else
            return 0;
      }

      double final_testset_performance() const
      {
         if (!training_record.empty())
            return training_record.end()->test_perf();
         else
            return 0;
      }
