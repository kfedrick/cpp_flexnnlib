/*
 * ActorCriticTrainer.cpp
 *
 *  Created on: Mar 22, 2015
 *      Author: kfedrick
 */

#include "ActorCriticTrainer.h"
...
namespace flexnnet
{
   ...
   const long ActorCriticTrainer::default_max_epochs = 1;
   const double ActorCriticTrainer::default_learning_momentum = 0.0;
   const double ActorCriticTrainer::default_performance_goal = 0.0;
   const double ActorCriticTrainer::default_min_gradient = 0.0;
   const int ActorCriticTrainer::default_max_valid_fail = 10;
   const bool ActorCriticTrainer::default_batch_mode = true;
   const unsigned int ActorCriticTrainer::default_batch_size = 0;
   const int ActorCriticTrainer::default_report_frequency = 1;

   const string ActorCriticTrainer::delta_network_weights_id =
      "delta_network_weights";
   const string ActorCriticTrainer::previous_delta_network_weights_id =
      "previous_delta_network_weights";
   const string ActorCriticTrainer::adjusted_network_weights_id =
      "adjusted_network_weights";
   const string ActorCriticTrainer::best_weights_id = "best_weights";
   const string ActorCriticTrainer::failback_weights_id = "failback_weights";

   ActorCriticTrainer::ActorCriticTrainer (ActorCriticNet &_model) :
   ...
   ActorCriticTrainer::~ActorCriticTrainer ()
   {
      // TODO Auto-generated destructor stub
   }

   NetworkWeightsData &ActorCriticTrainer::get_cached_network_weights (
   ...
   void ActorCriticTrainer::set_global_learning_rate (double _rate)
   {
   ...
   double ActorCriticTrainer::sim (EnvironmentSimulator *_env,
   ...
   double ActorCriticTrainer::sim_critic (EnvironmentSimulator *_env,
                                          unsigned int _sampleCount)
   {
      unsigned int step = 0;
      Pattern state;
   ...
   void ActorCriticTrainer::init_train ()
   {
...
// Restore weights to the best training epoch
   ...
   void ActorCriticTrainer::train2 (EnvironmentSimulator *_trainingEnv,
                                    double _objVal)
   {
      double perf;
      double global_performance = 0.5, prev_global_performance = 9999999.0;
      ...
      for (unsigned int epoch = 0; epoch < max_training_epochs; epoch++)
      {
         ...
         train_critic (_trainingEnv, 3 * actor_batch_size);
 ...
 */
   ...
   double ActorCriticTrainer::train_critic (EnvironmentSimulator *_trainingEnv,
   ...
   double ActorCriticTrainer::train_actor (EnvironmentSimulator *_trainingEnv,
   ...
   void ActorCriticTrainer::init_training_epoch ()
   {
      actor_critic_model.clear_error ();
      zero_delta_network_weights ();
      zero_network_eligibility_trace ();
 ...
 */
   double ActorCriticTrainer::train_exemplar (EnvironmentSimulator *_env,
...
// Restore model stochastic action flag setting
   ...
   void ActorCriticTrainer::calc_network_adj (const BaseNeuralNet &_net,
   ...
   void ActorCriticTrainer::calc_layer_bias_adj (const BaseNeuralNet &_net,
...
// Get the learning rates for the biases
   ...
   void ActorCriticTrainer::calc_layer_weight_adj (const BaseNeuralNet &_net,
...
// Get the learning rates for the weights
      ...
      /*
       if (print_gradient)
       {
       cout << " >>>> BaseLayer weight learning rate <<<<" << endl;
       for (unsigned int out_ndx = 0; out_ndx < layer.size(); out_ndx++)
       {
       for (unsigned int in_ndx = 0; in_ndx < layer_input_size; in_ndx++)
       {
   ...
   void ActorCriticTrainer::save_weights (const string &_id)
   {
...
// TODO - check the buffer id to make sure it's not one of the reserved values
   ...
   void ActorCriticTrainer::restore_weights (const string &_id)
   {
...
// TODO - check the buffer id to make sure it's not one of the reserved values
   ...
   void ActorCriticTrainer::alloc_network_weights_cache_entry (const string &_id)
   {
   ...
   void ActorCriticTrainer::apply_delta_network_weights ()
   {
      apply_delta_network_weights (*actor_critic_model.get_actor ());
      apply_delta_network_weights (*actor_critic_model.get_adaptive_critic ());
   }

   void ActorCriticTrainer::apply_delta_network_weights (BaseNeuralNet &_net)
   {
...
// TODO - decide if we should save off the current delta weights here
   ...
   void ActorCriticTrainer::zero_delta_network_weights ()
   {
      zero_delta_network_weights (*actor_critic_model.get_actor ());
      zero_delta_network_weights (*actor_critic_model.get_adaptive_critic ());
   }

   void ActorCriticTrainer::zero_delta_network_weights (const BaseNeuralNet &_net)
   {
...
// Iterate through the weights for each layer
   ...
   int ActorCriticTrainer::urand (int n)
   {
   ...
   inline
   void ActorCriticTrainer::save_network_eligibility_trace ()
   {
   ...
   inline
   void ActorCriticTrainer::update_network_eligibility_trace ()
   {
   ...
   void ActorCriticTrainer::zero_network_eligibility_trace ()
   {
   ...
   void ActorCriticTrainer::alloc_network_learning_rates ()
   {
      const BaseNeuralNet &actor = *actor_critic_model.get_actor ();
      const BaseNeuralNet &critic = *actor_critic_model.get_adaptive_critic ();
   ...
   void ActorCriticTrainer::reduce_learning_rates (double val)
   {
      const BaseNeuralNet &actor = *actor_critic_model.get_actor ();
      const BaseNeuralNet &critic = *actor_critic_model.get_adaptive_critic ();
